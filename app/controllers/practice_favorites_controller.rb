class PracticeFavoritesController < ApplicationController
    ...
    redirect_to practice_diaries_url, success: "#{practice_favorite.practice_diary.user.user_name}さんの練習記録をお気に入り登録しました！"
	end
	
	def destroy
		practice_favorite = current_user.practice_favorites.find_by(id: params[:id]).destroy
    redirect_to practice_diaries_url, info: "#{practice_favorite.practice_diary.user.user_name}さんの練習記録をお気に入り解除しました！"
	end
end
