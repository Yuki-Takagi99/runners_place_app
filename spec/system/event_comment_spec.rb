...
RSpec.describe 'EventComment', type: :system do
  let!(:user) { create(:user) }
  let!(:other_user) { create(:jon) }
  let!(:event) { create(:event, user_id: user.id) }

  describe 'イベントのコメント作成', js: true do
    ...
    context 'イベント詳細画面でコメントが未入力の場合、' do
  ...
  describe 'コメントの編集・削除', js: true do
    context 'コメントの編集画面でデータが正しく入力されている場合' do
      it 'コメントが編集できること' do
        ...
        click_on 'コメント編集'
        # 編集のtext_areaを指定
        find(".materialize-textarea").set("参加予定です！よろしくお願いします！")
        click_on '送信'
        expect(page).to have_content '1件のコメントがあります！'
        expect(page).to have_content '参加予定です！よろしくお願いします！'
      end
    end
